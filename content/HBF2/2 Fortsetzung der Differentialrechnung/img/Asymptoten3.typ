...
#let asymptote = xs.map(x =>  (x - 2))
#let function11 = x11.map(x =>  (x*x - 3*x) / (x - 1))
#let function12 = x12.map(x =>  (x*x - 3*x) / (x - 1))
...
#lq.diagram(
  ...
  lq.plot(x11, function11, mark: none, stroke: thirdcolor + 1.5pt, label: [$display(f(x) = (x^2 - 3x) / (x - 1))$]),
